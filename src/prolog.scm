...
(begin
  ...
  (define-predicate (--add-solution-with-vars-and-fail template . vars)
    (let* ((bindings (current-bindings))
           (substituted-template (substitute-bindings bindings template))
           (substituted-vars (map (lambda (v) (substitute-bindings bindings v)) vars))
  ...
  (define-predicate (bagof template goal result-bag)
    ...
    (define (enumerate-groups grouped-solutions free-vars result-bag initial-bindings)
      (if (null? grouped-solutions)
          ...
          (let* ((current-group (car grouped-solutions))
                 ...
                 (next-group-thunk
                  (lambda ()
                    (enumerate-groups (cdr grouped-solutions) free-vars result-bag initial-bindings)))
                 (b1 (unify free-vars free-var-values initial-bindings))
    ...
    (let* ((initial-bindings (current-bindings))
           (goal-vars (variables-in (substitute-bindings initial-bindings goal)))
           (template-vars (variables-in (substitute-bindings initial-bindings template)))
           ...
           (all-solutions
             (parameterize ((current-solution-accumulator '()))
               (prove-all `(,goal ,collector-goal fail) initial-bindings)
               (reverse (current-solution-accumulator))))
           (grouped-solutions (group-solutions all-solutions)))
       (enumerate-groups grouped-solutions free-vars result-bag initial-bindings)))
