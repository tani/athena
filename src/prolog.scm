...
(begin
  ...
  (define (unify-vars vars vals bindings)
    (if (null? vars)
        bindings
        (let ((b (unify (car vars) (car vals) bindings)))
          (if (failure? b)
              b
              (unify-vars (cdr vars) (cdr vals) b)))))
  ...
  (define (group-solutions pairs)
    (let loop ((ps pairs) (acc '()))
      (if (null? ps)
          ...
          (let* ((vals (caar ps))
                 (tmpl (cdar ps))
                 (existing (assoc vals acc)))
            (if existing
                (let* ((rest (alist-delete vals acc equal?))
                       (new-entry (cons vals (append (cdr existing) (list tmpl)))))
                  (loop (cdr ps) (cons new-entry rest)))
                (loop (cdr ps) (cons (cons vals (list tmpl)) acc)))))))
  ...
  (define-predicate (bagof template goal result-bag)
    (let* ((goal-vars (variables-in (substitute-bindings (current-bindings) goal)))
           ...
           (collector (cons '--add-solution-with-vars-and-fail (cons template free-vars))))
      (parameterize ((current-solution-accumulator '()))
        (let ((new-goals (list goal collector 'fail)))
          (prove-all new-goals (current-bindings)))
        (let* ((pairs (reverse (current-solution-accumulator)))
               (groups (group-solutions pairs)))
          (letrec ((enumerate
                    (lambda (gs)
                      (if (null? gs)
                          (make-failure)
                          (let* ((vals (caar gs))
                                 (bag (cdar gs))
                                 (next (lambda () (enumerate (cdr gs))))
                                 (b1 (unify-vars free-vars vals (current-bindings)))
                                 (b2 (unify result-bag bag b1)))
                            (if (failure? b2)
                                (next)
                                (let* ((result (prove-all (current-remaining-goals) b2))
                                       (cont (success-continuation result))
                                       (new-cont (combine cont next)))
                                  (make-success (success-bindings result) new-cont))))))))
            (enumerate groups))))))
          ...
          (letrec ((enumerate
                    (lambda (gs)
                      (if (null? gs)
                          ...
                          (let* ((vals (caar gs))
                                 (bag (cdar gs))
                                 (unique (delete-duplicates bag equal?))
                                 (sorted (list-sort sort-predicate unique))
                                 (next (lambda () (enumerate (cdr gs))))
                                 (b1 (unify-vars free-vars vals (current-bindings)))
                                 (b2 (unify result-set sorted b1)))
                            (if (failure? b2)
                                (next)
                                (let* ((result (prove-all (current-remaining-goals) b2))
                                       (cont (success-continuation result))
