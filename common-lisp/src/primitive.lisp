...
(define-predicate (or-2 goal1 goal2)
  (let* ((original-bindings *current-bindings*)
         (remaining-goals *current-remaining-goals*)
         (goals-1 (cons `(call ,goal1) remaining-goals))
         (result-1 (prove-goal-sequence goals-1 original-bindings))
         (goals-2 (cons `(call ,goal2) remaining-goals))
         (try-goal-2 (lambda () (prove-goal-sequence goals-2 original-bindings))))
    (if (failure-p result-1)
        (prove-goal-sequence goals-2 original-bindings)
        (let* ((success-bindings-1 (success-bindings result-1))
               (success-continuation-1 (success-continuation result-1))
               (new-continuation (merge-continuations success-continuation-1 try-goal-2)))
